        ...
        while ((type = in.next()) != END_DOCUMENT) {
            if (type == START_TAG) {
                ...
                if (TAG_ROOT_PATH.equals(tag)) {
                    ...
                    target = buildPath(new File(context.getFilesDir()), path);
                } else if (TAG_CACHE_PATH.equals(tag)) {
                    target = buildPath(new File(context.getCacheDir()), path);
                } else if (TAG_EXTERNAL.equals(tag)) {
                    target = buildPath(new File(Environment.getExternalStorageDirectory()), path);
                }
                if (target != null) {
